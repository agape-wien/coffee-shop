...
- All four `type="time"` TextFields in `OrdersSection` replaced with `DesktopTimePicker` (`ampm={false}` for 24h format)
- All four `type="date"` TextFields replaced with `DesktopDatePicker` (`format="DD.MM.YYYY"` matches German locale used in order list)
- Root cause of missing icons: `MobileDatePicker` / `MobileTimePicker` in MUI X v7 never render an icon button at all — the entire field is the touch target by design. `openPickerButtonPosition` is silently ignored. Fix: use `DesktopDatePicker` / `DesktopTimePicker`, which always render the icon button in the field and open a popper (appropriate for the desktop management screen).
- Icon supplied via `slots={{ openPickerIcon: CalendarTodayIcon }}` / `slots={{ openPickerIcon: AccessTimeIcon }}`
